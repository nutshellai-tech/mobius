# Mobius · 自进化的智能体操作系统

> 试图构建一个一劳永逸的完美 AI Harness 系统，就像试图寻找莫比乌斯环的尽头，终究是徒劳。

Mobius 是（据我们所知）全球首个**自进化**的开源智能体操作系统 —— 一个能够按个性化需求持续自我迭代的 AI 工作台。在这里，你的角色从「AI 使用者」转变为「智能体指挥官」：任意数量的开发项目、任意数量的服务器 / PC / 嵌入式设备、任意数量的普通任务（Issue）与研究任务（Research），一句话，一个想法，立刻在合适的位置生根发芽。

- 🔗 源码仓库：[nutshellai-tech/mobius](https://github.com/nutshellai-tech/mobius)
- 🌐 官网：[mobius.nutshellai.cn](https://mobius.nutshellai.cn/)

## 快速开始

### I-五分钟精通

- [万能捷径：小莫助理](tutorial/13_xiaomo_control_hub.md)
- [概念入门：导入项目并开启首个任务](tutorial/02a_import_project_and_begin_first_job.md)
- [概念入门：技能与记忆](tutorial/03a_using_skill_and_memory.md)

### II-高级能力

- [添加远程算力](tutorial/04_add_remote_server.md)
- [从个人电脑连接（aimux）](tutorial/27_aimux_from_pc.md)
- [使用小莫助手 Web 端](tutorial/05_using_xiaomo_assistant.md)
- [防止 Agent 偷懒（巡检 / 鞭策）](tutorial/06_prevent_agent_idle.md)
- [屏蔽无关 Skill 和 Memory](tutorial/07_exclude_skills_memory.md)
- [查看当前知识](tutorial/28_view_current_knowledge.md)
- [迁移 Skill 和 Memory](tutorial/12_migrate_skill_memory.md)
- [跨会话搜索内容](tutorial/25_search_session_content.md)
- [切换工作区布局](tutorial/29_switch_workspace_layout.md)

### III-管理员基础

- [添加用户](tutorial/01_add_user.md)
- [在管理中心监控智能体](tutorial/08_monitor_agents.md)
- [配置 Codex + 模型](tutorial/09_config_codex_model.md)
- [配置 Claude Code + 模型](tutorial/10_config_claude_code_model.md)
- [限制模型调用频率](tutorial/11_limit_model_usage.md)

### IV-自我进化能力

- [简单自进化示例](self-evo-demo.md)

### V-小技巧

- [聊天输入小技巧](tutorial/14_chat_input_tips.md)
- [中途切换模型](tutorial/15_switch_model_mid_session.md)
- [修改前端色彩主题](tutorial/16_change_frontend_theme.md)
- [标记重点项目（收藏）](tutorial/17_favorite_project.md)
- [项目文件打包下载](tutorial/18_package_download_project.md)
- [网页终端：打开目录终端，也能直看 Agent 后台](tutorial/26_web_terminal.md)

### VI-深度研究

- [创建研究任务（启用 Research 系统）](tutorial/19_enable_research_and_create.md)
- [组建智能体研究团队](tutorial/20_research_agent_team.md)
- [启用 Graph 绘图 Agent](tutorial/21_research_graph_agent.md)
- [启用图像绘制 Agent](tutorial/22_research_image_agent.md)
- [创建自定义团队成员](tutorial/23_custom_team_member.md)
- [更换成员形象](tutorial/24_change_member_avatar.md)
