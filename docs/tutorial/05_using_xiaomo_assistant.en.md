# How to Use the Xiaomo Assistant (Web)

‍

Xiaomo (小莫) is the lightweight assistant built into Mobius — it floats at the **bottom-right of every page**. Open it to ask, follow up, spawn clones, attach files, or go hands-free with voice — no need to create a project or task first. This walkthrough covers the whole flow.
...
- **Preset**: predefine a Xiaomo session template to reuse later (see section 7).
- **Expand / Fullscreen**: compact → expanded → fullscreen, cycling through three sizes.
- **Close**: collapse the panel (the conversation is preserved).

## 5. Spawn a "Clone Xiaomo" to Run a Task in Parallel
...
- **Expand input**: enlarge the box for longer messages.
- At the top there's also a "**Enable reply voice playback**" toggle — turn it on and Xiaomo will read each reply aloud (voice is selectable).

## 7. Preset (Advanced, Skippable)
...
> In one sentence: Xiaomo is an always-on helper — **open it anytime, ask anytime, and spawn a clone when you need to parallelize**.
