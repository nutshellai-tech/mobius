...
try {
  ...
  assert.match(context.prompt, new RegExp(`/runs/${snapshot.run.id}/scheduling`))
  assert.match(context.prompt, /fill_parallel_wave/)
  assert.match(context.prompt, new RegExp(`/runs/${snapshot.run.id}/result-events/<result-event-id>/ack`))
  assert.match(context.prompt, /"last_seen_seq": 123/)
  assert.match(context.prompt, /data_only/)
  assert.match(context.prompt, /Deduplicate repeated notifications by event_id and seq/)
  ...
  assert.match(context.prompt, /accepted completion submission is a terminal action/)
  assert.match(context.prompt, /"schema_version": "1.2"/)
  assert.match(context.prompt, /"outputs": \[/)
  assert.match(context.prompt, /"synthesis_manifest": \{/)
  assert.match(context.prompt, /"included_result_event_ids": \[/)
  assert.match(context.prompt, /"criterion_sources": \[/)
  assert.match(context.prompt, /"coverage_gaps": \[/)
  assert.match(context.prompt, /"name": "最终结果"/)
  assert.match(context.prompt, /keep summary short/)
  assert.match(context.prompt, /finalize_not_ready/)
  assert.match(context.prompt, /root and Run remain running/)
