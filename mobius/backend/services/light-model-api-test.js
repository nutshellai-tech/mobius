/**
 * light-model-api-test.js — 仅给管理员「测试连接」按钮使用.
 *
...
function describeToolCall(args) {
...
function validateToolCallArgs(args) {
...
function buildOpenAiChatBody(model) {
  return {
    model,
...
function buildOpenAiResponseBody(model) {
  return {
    model,
    input: USER_PROMPT,
    tools: [
...
function buildClaudeBody(model) {
  return {
    model,
    max_tokens: 256,
    messages: [{ role: 'user', content: USER_PROMPT }],
...
async function callOpenAiChatCompletion(baseUrl, apiKey, model) {
...
async function callOpenAiResponse(baseUrl, apiKey, model) {
...
async function callClaudeMessage(baseUrl, apiKey, model) {
...
async function testLightModelApi({ type, baseUrl, apiKey, model }) {
