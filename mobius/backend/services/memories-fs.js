/**
 * memories-fs.js — 文件系统驱动的 Memory (记忆) 存取.
 *
...
function userDefaultDir(userId) { return path.join(ROOT, `user=${userId}`, 'default_project'); }
function userProjectDir(userId, projectId) { return path.join(ROOT, `user=${userId}`, `project=${projectId}`); }
...
function readMemoryFromFile(file) {
...
function encodeUserId(userId, slug) { return `user:${userId}:${slug}`; }
function encodeProjectId(userId, projectId, slug) { return `project:${userId}:${projectId}:${slug}`; }
...
function shapeProject(m, userId, projectId) {
  return {
...
function buildMemoryFile({ name, description, body }) {
...
function ensureMemoryFileWithinLimit(raw) {
...
function targetPathFor({ userId, projectId, slug }) {
...
function create({ userId, projectId, name, description, body }) {
...
function upsertProjectMemory({ userId, projectId, slug, name, description, body }) {
...
function update({ id, name, description, body }) {
  const parsed = parseMemoryId(id);
...
function moveMemory({ id, requesterUserId, isAdmin, targetScope, targetProjectId }) {
  const parsed = parseMemoryId(id);
  if (!parsed) return { ok: false, error: 'id 非法' };
  if (!isSafeSlug(parsed.slug)) return { ok: false, error: 'id 非法' };
...
function walkMarkdownFiles(root, { maxFiles = 500, maxDepth = 8 } = {}) {
  const out = [];
