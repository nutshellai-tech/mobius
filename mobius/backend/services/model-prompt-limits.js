...
const { DEFAULT_WINDOW_HOURS, normalizeHours } = require('./agent-prompt-events')
...
function sinceForWindow(hours = WINDOW_HOURS) {
  const h = normalizeHours(hours, WINDOW_HOURS)
  return db.prepare("SELECT strftime('%Y-%m-%dT%H:%M:%fZ','now', ?) AS since").get(`-${h} hours`).since
}

function sinceForMinutes(minutes = WINDOW_MINUTES) {
  const m = Math.max(1, Math.round(Number(minutes) || WINDOW_MINUTES))
  return db.prepare("SELECT strftime('%Y-%m-%dT%H:%M:%fZ','now', ?) AS since").get(`-${m} minutes`).since
}

function optionFromResolved(resolved) {
  return {
    key: resolved.key,
...
function storageValuesForModel(modelOrKey) {
...
function countPromptsForModelSince(modelOrKey, modifier, userId = null) {
...
function countUserPromptsForModelSince(userId, modelOrKey, hours = WINDOW_HOURS) {
  if (!userId) return 0
  const h = normalizeHours(hours, WINDOW_HOURS)
  return countPromptsForModelSince(modelOrKey, `-${h} hours`, userId)
}

function countAllPromptsForModelSinceHours(modelOrKey, hours = WINDOW_HOURS) {
  const h = normalizeHours(hours, WINDOW_HOURS)
  return countPromptsForModelSince(modelOrKey, `-${h} hours`)
}

function countUserPromptsForModelSinceMinutes(userId, modelOrKey, minutes = WINDOW_MINUTES) {
  if (!userId) return 0
  const m = Math.max(1, Math.round(Number(minutes) || WINDOW_MINUTES))
  return countPromptsForModelSince(modelOrKey, `-${m} minutes`, userId)
}

function countAllPromptsForModelSinceMinutes(modelOrKey, minutes = WINDOW_MINUTES) {
  const m = Math.max(1, Math.round(Number(minutes) || WINDOW_MINUTES))
  return countPromptsForModelSince(modelOrKey, `-${m} minutes`)
}

function modelLimitConfigForKey(modelKey) {
  try {
...
function configPathForOption(opt) {
...
function activeTmuxWindowCountForModel(modelOrKey) {
  let resolved
  try {
  ...
  const values = new Set(storageValuesForModel(resolved.key))
  let count = 0
  for (const win of windows) {
    ...
    let row
    try {
      row = db.prepare('SELECT model FROM sessions_v2 WHERE session_id = ?').get(sessionId)
    } catch {
      row = null
    }
...
function limitState(count, limit) {
  ...
  const effectiveLimit = Number.isFinite(normalized) ? Math.max(0, Math.floor(normalized)) : null
  return {
    count,
...
function usageEntryForUserModel(userId, modelOrKey, hours = WINDOW_HOURS) {
...
function usageForUser(userId, hours = WINDOW_HOURS) {
  const h = normalizeHours(hours, WINDOW_HOURS)
  const models = {}
...
function adminLimitsPayload() {
  const limits = adminSettings.getModelPromptLimits()
  return {
    window_hours: WINDOW_HOURS,
    window_minutes: WINDOW_MINUTES,
    models: modelRegistry.listSessionModelOptions().map((opt) => ({
      key: opt.key,
      value: opt.value,
      model: opt.model,
      label: opt.label,
...
function checkCreateAllowed(userId, modelOrKey) {
