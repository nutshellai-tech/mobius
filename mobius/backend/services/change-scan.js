...
const { Issues, IssueIntegrations } = require('../repositories/issues');
...
function extractChangedFiles(sessionId, manualFiles = []) {
  const files = new Map();
  const add = (raw, meta = {}) => {
...
function summarizeSessionChange(session, files) {
...
function recomputeProjectConflicts(projectId) {
...
function refreshIssueIntegration(issueId) {
...
function changePayload(changeId) {
...
function scanSessionChange(req, session, manualFiles = []) {
...
function integrateIssue(req, issueId) {
  const issue = Issues.findById(issueId);
