/**
 * inactive-tmux-cleaner.ts — 自动清理不活跃的 tmux agent window.
 *
 * 范围:
...
import { db } from '../../db';
import { BACKEND_WORKER_LOG_DIR, DEFAULT_AGENT_BACKEND, MODEL_OPTIONS } from '../config';
import { Messages } from '../repositories/messages';
import { flagDirOf, runningFlagPathOf, safeRemoveFlagDir } from '../utils/session-flags';
...
function iso(ms: number | null | undefined): string {
  return Number.isFinite(ms as number) && (ms as number) > 0 ? new Date(ms as number).toISOString() : '(unknown)';
}
...
function parseDbTime(value: any): number | null {
...
function source(ms: number | null | undefined, label: string): ActivitySource | null {
  return ms ? { ms, label } : null;
}

function latestActivity(sources: (ActivitySource | null)[]): ActivitySource | null {
  const valid = sources.filter((x): x is ActivitySource => !!x && Number.isFinite(x.ms) && x.ms > 0);
...
function loadDbSessionsById(): Map<string, DbSessionRow> {
...
function backendNames(): string[] {
  return Array.from(new Set([
    ...
    ...Object.values(MODEL_OPTIONS || {}).map((m: any) => m.backend).filter(Boolean),
  ]));
}
...
function activityForWindow(backend: any, windowInfo: any, dbSession: DbSessionRow | null): { latest: ActivitySource | null; runtime: any; sources: ActivitySource[] } {
  ...
  return {
    latest,
    runtime,
    sources: sources.filter(Boolean) as ActivitySource[],
  };
}

function cleanupFlagDir(sessionId: string, dbSession: DbSessionRow | null, runtime: any): boolean {
...
function completionFlagState(sessionId: string, dbSession: DbSessionRow | null, runtime: any): { known: boolean; completed: boolean; root: string | null; checked: any[] } {
  const roots = new Set<string>();
  if (runtime?.flagRoot) roots.add(runtime.flagRoot);
  if (runtime?.cwd) roots.add(runtime.cwd);
  if (dbSession?.bind_path) roots.add(dbSession.bind_path);
...
function thresholdText(ms: number): string {
...
function markDbIdle(sessionId: string): void {
...
function writeCleanupMessage(sessionId: string, backendName: string, windowInfo: any, lastActivity: ActivitySource | null, ageMs: number, cleanupReason: string, thresholdMs: number): void {
  try {
...
async function scanOnce(): Promise<void> {
  ...
  const candidates: any[] = [];        // 命中清理阈值的窗, 等主循环结束后统一处理
  ...
  try {
    ...
    for (const f of candidates) {
      ...
      const sources = f.activity.sources
        .map((x: ActivitySource) => `${x.label}=${iso(x.ms)}`)
        .join(', ');
...
function startInactiveTmuxCleaner(): NodeJS.Timeout | null {
