/**
 * project-knowledge.js — 把项目绑定路径下的 .imac/project_knowledge.md
...
function parseHistoryTimestamp(filename) {
...
function snapshotProjectKnowledge(project, content) {
...
function cleanupHistorySnapshots(dir) {
  try {
    const entries = fs.readdirSync(dir)
      ...
      .filter(Boolean)
      .filter(e => e.filename.startsWith(HISTORY_FILE_PREFIX) && e.filename.endsWith(HISTORY_FILE_SUFFIX));
...
function listProjectKnowledgeHistory(project) {
  const dir = projectKnowledgeHistoryDir(project);
  if (!dir) return [];
  try {
    if (!fs.existsSync(dir)) return [];
    return fs.readdirSync(dir)
      .filter(f => f.startsWith(HISTORY_FILE_PREFIX) && f.endsWith(HISTORY_FILE_SUFFIX))
      .map(filename => {
        const fullPath = path.join(dir, filename);
        let stat;
...
function readProjectKnowledgeHistoryFile(project, filename) {
  const dir = projectKnowledgeHistoryDir(project);
  if (!dir) return { ok: false, error: '项目未绑定路径' };
  // 严格校验文件名, 避免路径穿越.
...
function syncProjectKnowledge(project, opts = {}) {
...
function syncProjectKnowledgeForProjectId(projectId, opts = {}) {
...
// 已存在则不覆盖, 保护用户已有内容.
function ensureProjectKnowledgeFile(project) {
