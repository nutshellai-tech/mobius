...
function cleanRemoteName(value, { required = true } = {}) {
...
function timeoutToSshSeconds(value) {
...
function runAimux(args, { timeoutMs = 60000 } = {}) {
...
function remoteBrowseScript(targetPath) {
  return [
    'IFS= read -r TARGET',
    targetPath,
...
function parseBrowseOutput(stdout) {
  const out = { path: '', parent: '', entries: [] };
...
async function testRemote(name, timeout) {
...
async function hardwareRemote(name, timeout) {
  const remoteName = cleanRemoteName(name);
...
async function browseRemotePath(name, remotePath, timeout) {
  const remoteName = cleanRemoteName(name);
...
async function addRemote({ host, user, port, name, identity, timeout } = {}) {
  ...
  if (!result.ok) {
    const err = new Error(resultMessage(result));
    err.result = result;
    throw err;
  }
  return {
