/**
 * session-next-question-predictor.js
 *
...
const { buildSessionTransferMarkdown } = require('./session-transfer');
...
function clipForPrompt(markdown) {
...
function buildUserPrompt({ session, transfer }) {
...
function questionArrayFromArgs(args) {
...
async function callOpenAiChatCompletion(cfg, prompt) {
...
async function callClaudeMessage(cfg, prompt) {
...
async function callLightModel(cfg, prompt) {
  const handlers = {
...
async function predictNextQuestionsForSession({ session, jsonlPath }) {
