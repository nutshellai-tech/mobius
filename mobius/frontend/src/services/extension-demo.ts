...
export const EXTENSION_DEMO_STATE_KEY = 'imac-demo:mobius-dot-logo-space-extension'
...
export const EXTENSION_DEMO_DEFAULTS = {
  ...
  projectDescription: '高级引导路线：检查并迭代一个可运行的莫比乌斯光点标志空间拓展。',
...
export function createExtensionDemoState(): ExtensionDemoState {
  return {
    active: true,
    startedAt: Date.now(),
    ...EXTENSION_DEMO_DEFAULTS,
  }
}

function normalizeExtensionDemoState(value: unknown): ExtensionDemoState | null {
  if (!value || typeof value !== 'object') return null
  return {
    ...EXTENSION_DEMO_DEFAULTS,
    ...(value as Partial<ExtensionDemoState>),
  }
}

export function readExtensionDemoState(): ExtensionDemoState | null {
  try {
    const raw = sessionStorage.getItem(EXTENSION_DEMO_STATE_KEY)
    if (!raw) return null
    const parsed = JSON.parse(raw)
    return normalizeExtensionDemoState(parsed)
  } catch {
    return null
  }
}

export function writeExtensionDemoState(state: ExtensionDemoState) {
  try {
    sessionStorage.setItem(EXTENSION_DEMO_STATE_KEY, JSON.stringify(state))
  } catch {}
}

export function patchExtensionDemoState(patch: Partial<ExtensionDemoState>) {
  const current = readExtensionDemoState() || createExtensionDemoState()
  writeExtensionDemoState({ ...current, ...patch })
}

export function completeExtensionDemoState() {
  patchExtensionDemoState({ active: false, completedAt: Date.now() })
}

export function extensionDemoIsActive() {
  return !!readExtensionDemoState()?.active
}

export function isExtensionDemoProject(projectId?: string) {
  const state = readExtensionDemoState()
  return !!state?.active && !!projectId && state.projectId === projectId
}

export function isExtensionDemoIssue(issueId?: string) {
  const state = readExtensionDemoState()
  return !!state?.active && !!issueId && state.issueId === issueId
}

export function isExtensionDemoSession(sessionId?: string) {
  const state = readExtensionDemoState()
