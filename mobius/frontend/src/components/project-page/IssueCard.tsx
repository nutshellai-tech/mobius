  ...
  return (
    <div
      ...
      <div className="px-4 py-3 border-b flex items-start gap-2" style={{ borderColor: 'var(--border-color)' }}>
        ...
        {canManage && <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
          ...
          <button onClick={() => onConfirm({ kind: issue.pinned ? 'unpin' : 'pin', issue })} className="p-1 rounded hover:bg-white/10" title={issue.pinned ? '取消置顶' : '置顶'}>
            <svg className="w-3.5 h-3.5" style={{ color: issue.pinned ? '#f59e0b' : 'var(--text-muted)' }} fill={issue.pinned ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></svg>
          </button>
