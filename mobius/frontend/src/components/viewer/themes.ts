...
export const DEFAULT_THEME: CardTheme = { dot: 'bg-gray-500', border: 'border-gray-500/15', bg: 'bg-gray-500/[0.04]', text: 'text-gray-400', label: 'entry' }

// 特例: assistant 里带 name:"Edit" 的 tool_use 卡片 — 用 indigo (与 assistant 蓝相邻但可区分),
// 边框/底色比常规 type 稍重一点, 方便在长列表里一眼扫到文件改动.
export const EDIT_TOOL_THEME: CardTheme = { dot: 'bg-indigo-400', border: 'border-indigo-500/20', bg: 'bg-indigo-500/[0.07]', text: 'text-indigo-300', label: 'file·edit' }
...
export const START_PY_THEME: CardTheme = { dot: 'bg-yellow-400', border: 'border-yellow-500/20', bg: 'bg-yellow-500/[0.07]', text: 'text-yellow-300', label: 'start.py' }

// 特例: assistant 里带 name:"Bash" 的 tool_use 卡片 (Claude Code shell 调用).
// 用 cyan, 呼应终端/控制台意象, 与 Edit indigo、start.py yellow 都拉开, 长列表里可识别.
export const BASH_TOOL_THEME: CardTheme = { dot: 'bg-cyan-400', border: 'border-cyan-500/20', bg: 'bg-cyan-500/[0.06]', text: 'text-cyan-300', label: 'bash' }

// 特例: assistant 里带 name:"Read" 的 tool_use 卡片.
// 用 sky, 与 Bash cyan / Edit indigo 近邻但可区分, 方便扫文件读取操作.
export const READ_TOOL_THEME: CardTheme = { dot: 'bg-sky-400', border: 'border-sky-500/20', bg: 'bg-sky-500/[0.06]', text: 'text-sky-300', label: 'read' }

// 特例: event_msg.payload.type === 'context_compacted' 的卡片 — 一次上下文压缩事件,
// 在长列表里需要一眼可扫, 复用 yellow (gold) 与 start.py 同色但 label 区分.
export const CONTEXT_COMPACTED_THEME: CardTheme = { ...START_PY_THEME, label: 'ctx·compact' }

// 特例: Claude assistant 最终结束消息. 复用 system/turn_duration 的 amber gold 主题,
// 让 stop_reason:"end_turn" 的卡片在长列表中和轮次耗时卡片一样容易扫到.
export const ASSISTANT_END_TURN_THEME: CardTheme = { ...TYPE_THEME.system, label: 'end_turn' }
...
export const COMPACT_DONE_THEME: CardTheme = { ...TYPE_THEME.system, label: 'compacted' }

// 特例: user 消息里的 /goal 命令输出信号 (local-command-stdout 正文以 "Goal set" 开头).
// 与 compact 同款 amber gold, label 区分, 让"目标已设置"在长列表里一眼可扫.
export const GOAL_SET_THEME: CardTheme = { ...TYPE_THEME.system, label: 'goal-set' }
...
export const LOCAL_COMMAND_THEME: CardTheme = { ...TYPE_THEME.system, label: 'local-cmd' }

// 特例: assistant 响应文本命中这些关键词时整卡复用 gold 主题.
// 只检查 assistant 文本响应, 不把 tool_use/input.command 当作本规则的命中范围.
export const ASSISTANT_RESPONSE_KEYWORD_THEME: CardTheme = { ...START_PY_THEME, label: 'assistant·keyword' }

// 醒目主题: blackboard 相关消息用 fuchsia, 边框/底色比常规 type 重很多, 在长列表里一眼可见.
export const BLACKBOARD_THEME: CardTheme = { dot: 'bg-fuchsia-400', border: 'border-fuchsia-500/15', bg: 'bg-fuchsia-500/[0.05]', text: 'text-fuchsia-200', label: 'blackboard' }
