...
const { BLACKBOARD_MARKER, RUNNING_FLAG_REMINDER_MARKER, isBlackboardReminder, isRunningFlagReminder, isLocalCommandReminder, isNewRound } = helpers
...
// ── Claude Code 本地命令产物 (/compact 等): 套 user 外壳的工具产物, 不开新轮 ──
// 与 Blackboard / running-flag 同理: 一次 /compact 会产生 caveat / command-name / stdout 等多条
// user 消息, 都不是人类提问, 不应各自触发新 Round (否则会拆成 14.0/15.0/16.0 多个空轮次).
test('本地命令 stdout (compact 完成信号): type:user 字符串 content → 不开新轮', () => {
  const entry = {
    type: 'user',
    message: {
      role: 'user',
...
test('本地命令 stdout: type:user 数组 (text 块) content → 不开新轮', () => {
  const entry = {
    type: 'user',
    message: {
      role: 'user',
...
test('本地命令 caveat: <local-command-caveat> 包裹 → 不开新轮', () => {
  const entry = {
    type: 'user',
    message: {
      role: 'user',
  ...
  assert.equal(isNewRound(entry), false, 'local-command-caveat 不应触发新 Round')
})

test('本地命令本体: <command-name>/<command-message>/<command-args> → 不开新轮', () => {
  const entry = {
    type: 'user',
    message: {
      role: 'user',
      content: '<command-name>/compact</command-name> <command-message>compact</command-message> <command-args></command-args>',
    },
  }
  assert.equal(isNewRound(entry), false, 'command-name 等命令本体不应触发新 Round')
})
...
// ── isLocalCommandReminder 直接测试 ─────────────────────────────────────────
test('isLocalCommandReminder: 各类 local-command / command 标签都命中', () => {
  // compact 完成信号 (stdout + Compacted)
  assert.equal(isLocalCommandReminder({
    type: 'user',
...
test('isLocalCommandReminder: 普通用户消息不命中 (即便含 "compact" 字样但无标签包裹)', () => {
  // 没有 <local-command-*> / <command-*> 包裹的普通文本不命中, 且仍要开新轮 (回归保护, 不能误杀)
  assert.equal(isLocalCommandReminder({
    type: 'user',
...
test('isLocalCommandReminder: null/undefined 安全返回 false', () => {
  assert.equal(isLocalCommandReminder(null), false)
  assert.equal(isLocalCommandReminder(undefined), false)
  assert.equal(isLocalCommandReminder({}), false)
})
