...
//     LLM API Base URL（兼容 OpenAI 格式），必填。
//
...
const MODEL    = process.env.VIRTUAL_PET_LLM_MODEL || 'gpt-4o-mini';
...
function callQwen(messages) {
  if (!API_BASE) {
    return Promise.reject(new Error(
      '[virtual-pet] LLM API 未配置。请在项目根 .env 中设置 VIRTUAL_PET_LLM_API_BASE 和 VIRTUAL_PET_LLM_API_KEY。'
    ));
  }
