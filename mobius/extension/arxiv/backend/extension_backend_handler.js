 ...
 *   - list_topics                 → 返回所有主题
 *   - add_topic { name, query, schedule_cron?, interval_minutes? } → 新增 + 注册 cron
...
function actionAddTopic({ db, username, extDataDir, extMainPayload }) {
  ...
  const now = nowIso();
  db.prepare(`
    INSERT INTO topics (id, name, query, schedule_cron, interval_minutes, is_active, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, 1, ?, ?)
  `).run(id, name, query, scheduleCron, intervalMinutes, now, now);
  ...
  return { ok: true, topic: { ...topic, is_active: true, created_at: now, updated_at: now } };
}
