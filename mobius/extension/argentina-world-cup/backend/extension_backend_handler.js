/**
 * argentina-world-cup/backend/extension_backend_handler.js
 *
 * 阿根廷队世界杯视角内容聚合后端，职责：
 *   - 周期性抓取 RSS / feed（公开体育媒体 World Cup / Argentina football 频道）
...
const EXTENSION_NAME = 'argentina-world-cup';
...
const RSS_SOURCES = [
  {
    id: 'google-news-zh-argentina-worldcup',
    name: 'Google News 中文 · 阿根廷队世界杯',
    url: 'https://news.google.com/rss/search?q=%E9%98%BF%E6%A0%B9%E5%BB%B7%E9%98%9F%202026%E4%B8%96%E7%95%8C%E6%9D%AF%20OR%20%E9%98%BF%E6%A0%B9%E5%BB%B7%20%E6%A2%85%E8%A5%BF%20%E4%B8%96%E7%95%8C%E6%9D%AF%20when%3A14d&hl=zh-CN&gl=CN&ceid=CN:zh-Hans',
    priority: 8,
  },
  {
    id: 'google-news-zh-messi-scaloni',
    name: 'Google News 中文 · 梅西与斯卡洛尼',
    url: 'https://news.google.com/rss/search?q=%E6%A2%85%E8%A5%BF%20%E6%96%AF%E5%8D%A1%E6%B4%9B%E5%B0%BC%20%E9%98%BF%E6%A0%B9%E5%BB%B7%20%E4%B8%96%E7%95%8C%E6%9D%AF%20when%3A14d&hl=zh-CN&gl=CN&ceid=CN:zh-Hans',
    priority: 7,
  },
  {
    id: 'google-news-zh-argentina-fixtures',
    name: 'Google News 中文 · 阿根廷赛程与阵容',
    url: 'https://news.google.com/rss/search?q=%E9%98%BF%E6%A0%B9%E5%BB%B7%20%E8%B5%9B%E7%A8%8B%20%E9%98%B5%E5%AE%B9%20%E5%8A%B3%E5%A1%94%E7%BD%97%20%E9%98%BF%E5%B0%94%E7%93%A6%E9%9B%B7%E6%96%AF%20when%3A14d&hl=zh-CN&gl=CN&ceid=CN:zh-Hans',
    priority: 7,
  },
  {
    id: 'google-news-en-argentina',
    name: 'Google News · Argentina World Cup',
    url: 'https://news.google.com/rss/search?q=Argentina%20national%20team%20Messi%20Scaloni%202026%20World%20Cup%20when%3A14d&hl=en-US&gl=US&ceid=US:en',
    priority: 5,
  },
...
const WORLD_CUP_KEYWORDS = [
  ...
  // 阿根廷队视角关键词：用于把相关新闻往上排
  'argentina', 'argentine', 'albiceleste', 'selección argentina', 'seleccion argentina',
  '阿根廷', '阿根廷队', '潘帕斯雄鹰', '蓝白军团', '卫冕冠军',
  'messi', 'lionel messi', '梅西', 'scaloni', '斯卡洛尼',
  'lautaro', 'lautaro martinez', '劳塔罗',
  'julian alvarez', 'julián álvarez', '阿尔瓦雷斯',
  'emiliano martinez', 'emiliano martínez', '大马丁', '迪布',
  'de paul', '德保罗', 'enzo fernandez', 'enzo fernández', '恩佐',
  'mac allister', '麦卡利斯特', 'romero', '罗梅罗',
];
...
async function buildWorldCupAgentPrompt({ ext_data_dir, message, display_name, reuse }) {
  ...
  return [
    ...
    `会话类型：${reuse ? '继续既有 Argentina World Cup Agent Session' : '新建 Argentina World Cup Agent Session'}`,
    '',
    '## 你的角色',
    '- 你是莫比乌斯“阿根廷队 · 世界杯追踪”插件的专属 Agent。',
    '- 你可以回答阿根廷队在 2026 世界杯相关的赛程、阵容、球员、新闻、潜在对手、主办城市、页面数据来源等问题，默认用中文。',
    '- 如果用户要求修改这个阿根廷队世界杯插件，你可以直接检查并修改插件源码，不要只给建议。',
    '- 回答赛事事实时必须区分“公开数据已确认”和“页面兜底/推演/观察”，不要把示意内容说成正式赛果或官方名单。',
    '',
    '## 插件代码位置',
    `- 仓库根：${REPO_ROOT}`,
    `- 插件目录：${EXTENSION_DIR}`,
    ...
    '',
    '## 修改插件时必须遵守',
    '- 只处理 argentina-world-cup 插件相关文件，不要动 world-cup、france-world-cup、self-cognition、README 主仓库等无关文件。',
    '- 前端源文件改完后，同步 index.html/main.js/styles.css 到 frontend/dist/。',
    '- 能跑校验时至少执行 node --check backend/extension_backend_handler.js、node --check frontend/main.js、git diff --check -- mobius/extension/argentina-world-cup。',
    ...
    '</further-answering>',
    '- 如果用户只是问简单阿根廷队世界杯问题，further-answering 中直接给简明答案。',
    ...
    '',
    '请先判断用户是在问阿根廷队世界杯信息、追问数据来源，还是要求修改插件。需要修改时直接执行并在最后说明改了什么、如何验证、还有什么风险。',
  ].join('\n');
}
...
async function startWorldCupAgentChat({ username, display_name, ext_data_dir, payload, logger }) {
  ...
  if (reuse) {
    ...
    created = createExtensionAnalysisSession({
      ...
      extensionDisplayName: '阿根廷队世界杯问AI',
      projectDescription: '阿根廷队 2026 世界杯视角插件的 AI 问答与页面维护工作区。Agent 可以回答阿根廷队世界杯问题，也可以按用户要求修改 argentina-world-cup 插件。',
      issueTitle: '阿根廷队世界杯插件 AI 问答与维护',
      issueDescription: '接收阿根廷队赛事问答、素材更新、赛程新闻刷新、页面功能修改等任务。',
      sessionName: `阿根廷队世界杯问AI ${new Date().toISOString().slice(0, 16).replace('T', ' ')}`,
  ...
  const requestId = `argentina-world-cup-agent-${session.session_id}-${Date.now()}`;
  logger?.info?.(`argentina-world-cup agent queued: session=${session.session_id} request=${requestId}`);
  return {
    ok: true,
    async: true,
    status: 'queued',
    reply: '已发送到阿根廷队问AI。后台会在对应 Session 中回答；如果你要求修改插件，会直接处理 argentina-world-cup 插件文件。',
    ...
    __mobius_post_actions: [{
      ...
      source: 'extension.argentina-world-cup.agent_chat',
...
function placeholderContent() {
  ...
  return [
    {
      id: 'sample-1',
      dedupe_key: 'sample-argentina-title-defense',
      title: '阿根廷卫冕路线观察（示例数据）',
      summary: '这个页面以阿根廷队为核心视角追踪 2026 世界杯：重点关注梅西是否继续出战、斯卡洛尼的阵容更新、锋线搭配和淘汰赛潜在对手。真实新闻源恢复后会自动替换示例内容。',
    ...
    {
      id: 'sample-2',
      dedupe_key: 'sample-argentina-host-map',
      title: '阿根廷北美征程：从小组赛到纽约决赛（示例数据）',
      summary: '美加墨三国联办意味着阿根廷需要适应跨时区旅行、不同气候和大跨度场地切换。页面会把主办城市巡礼改造成阿根廷可能经过的路线图。',
    ...
    {
      id: 'sample-3',
      dedupe_key: 'sample-argentina-squad',
      title: '阿根廷阵容观察：核心传承与锋线选择（示例数据）',
      summary: '静态观察名单聚焦梅西、劳塔罗、阿尔瓦雷斯、恩佐、麦卡利斯特、德保罗、罗梅罗和大马丁；正式名单与射手榜会以后端同步的公开数据为准。',
