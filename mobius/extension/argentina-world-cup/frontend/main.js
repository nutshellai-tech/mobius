...
//       阿根廷射手渲染 / 中文新闻主区（左大卡 + 右列表）/ 球员 / 北美路线
//       调用后端 read 拉取真实 RSS 新闻、赛程、射手和同步状态
...
function updateCountdown() {
  ...
  if (now < kickoff) {
    ...
    if (prefixEl) prefixEl.textContent = '距离世界杯揭幕';
  } else if (now < final) {
    const ms = final - now;
    const d = Math.floor(ms / 86400000);
    const h = Math.floor((ms % 86400000) / 3600000);
    el.textContent = `${d} 天 ${h} 时`;
    if (prefixEl) prefixEl.textContent = '距离决赛之夜';
  } else {
    el.textContent = '已落幕';
    if (prefixEl) prefixEl.textContent = '阿根廷本届世界杯';
  }
}
updateCountdown();
...
function fallbackFixture(ymd, idx) {
  ...
  return {
    id: `fallback-${ymd}-${idx}`,
    date: preset?.date || `${ymd}T${String(hour).padStart(2, '0')}:00:00Z`,
    stage: preset?.stage || tournamentPhase(ymd),
    home: pair[0],
    homeName: pair[1],
    away: pair[2],
    awayName: pair[3],
    ...
    status: '',
    kind: 'upcoming',
    venue: preset?.venue || (idx % 2 ? 'Argentina Watch Desk' : 'La Albiceleste Hub'),
    city: preset?.city || (idx % 2 ? '阿根廷可能赛区' : '卫冕路径推演'),
    source: preset?.source || '阿根廷动态示意',
    sample: true,
  };
}
...
const TEAM_ACCENTS = {
  ...
  CAN: '#d80621', CHI: '#d52b1e', CIV: '#f77f00', CMR: '#007a5e', COD: '#00a3e0', COL: '#fcd116', CPV: '#003893', CRO: '#171796',
  DEN: '#c60c30', ECU: '#ffdd00', EGY: '#ce1126', ENG: '#cf142b', ESP: '#aa151b',
  FRA: '#0055a4', GER: '#dd0000', GHA: '#fcd116', ITA: '#008c45', JPN: '#bc002d',
  JOR: '#007a3d', KOR: '#0047a0', MAR: '#c1272d', MEX: '#006847', NED: '#ff4f00', NGA: '#008751',
...
const FOCUS_PLAYERS = [
  { rank: '队长', name: 'Lionel Messi', country: '阿根廷 · ARG', goals: '待赛', assists: '—', appearances: '—', role: '阿根廷队长 · 组织与终结' },
  { rank: '9号', name: 'Lautaro Martínez', country: '阿根廷 · ARG', goals: '待赛', assists: '—', appearances: '—', role: '中锋选择 · 禁区终结' },
  { rank: '前场', name: 'Julián Álvarez', country: '阿根廷 · ARG', goals: '待赛', assists: '—', appearances: '—', role: '前场压迫 · 纵深冲击' },
  { rank: '中场', name: 'Alexis Mac Allister', country: '阿根廷 · ARG', goals: '待赛', assists: '—', appearances: '—', role: '中场连接 · 节奏控制' },
  { rank: '中场', name: 'Enzo Fernández', country: '阿根廷 · ARG', goals: '待赛', assists: '—', appearances: '—', role: '中路推进 · 长传转移' },
  { rank: '中场', name: 'Rodrigo De Paul', country: '阿根廷 · ARG', goals: '待赛', assists: '—', appearances: '—', role: '攻守覆盖 · 情绪引擎' },
  { rank: '门将', name: 'Emiliano Martínez', country: '阿根廷 · ARG', goals: '待赛', assists: '—', appearances: '—', role: '门线核心 · 点球叙事' },
  { rank: '后防', name: 'Cristian Romero', country: '阿根廷 · ARG', goals: '待赛', assists: '—', appearances: '—', role: '中卫领袖 · 对抗强度' },
];
...
const MATCH_BACKDROPS = {
  'ARG-CPV': VISUALS.hardRock,
  'ARG-ALG': VISUALS.azteca,
  'ARG-AUT': VISUALS.sofi,
  'JOR-ARG': VISUALS.bmo,
  'ARG-MEX': VISUALS.azteca,
  'ARG-NED': VISUALS.metlife,
  'ARG-BRA': VISUALS.hardRock,
  'ARG-FRA': VISUALS.metlife,
  'ARG-ESP': VISUALS.att,
...
const HERO_FRAMES = [
  { label: 'Defending champion', title: '阿根廷卫冕线', meta: '蓝白军团 · 2026', img: VISUALS.trophy },
  { label: 'Captain watch', title: '梅西最后舞台', meta: '迈阿密 · 北美叙事', img: VISUALS.hardRock },
  { label: 'Scaloni board', title: '斯卡洛尼棋盘', meta: '阵容更新 · 中前场选择', img: VISUALS.soccerField },
  { label: 'Final dream', title: 'MetLife 决赛夜', meta: '纽约/新泽西 · 7.19', img: VISUALS.metlife },
  { label: 'Mexico echo', title: 'Azteca 历史回声', meta: '1986 记忆 · 2026 路线', img: VISUALS.azteca },
  { label: 'Travel map', title: '北美长途征程', meta: '美加墨 · 跨时区适应', img: VISUALS.northAmerica },
];

const CITY_PULSE = [
  { city: 'Miami', stadium: 'Hard Rock Stadium', tag: 'Messi base', img: VISUALS.hardRock },
  { city: 'New York / New Jersey', stadium: 'MetLife Stadium', tag: 'Final dream', img: VISUALS.metlife },
  { city: 'Mexico City', stadium: 'Estadio Azteca', tag: '1986 echo', img: VISUALS.azteca },
  { city: 'Los Angeles', stadium: 'SoFi Stadium', tag: 'West coast', img: VISUALS.sofi },
  { city: 'Dallas', stadium: 'AT&T Stadium', tag: 'Knockout hub', img: VISUALS.att },
  { city: 'Toronto', stadium: 'BMO Field', tag: 'Canada leg', img: VISUALS.bmo },
  { city: 'Vancouver', stadium: 'BC Place', tag: 'Pacific leg', img: VISUALS.bcPlace },
  { city: 'Monterrey', stadium: 'Estadio BBVA', tag: 'Mexico north', img: VISUALS.bbva },
];
...
const STANDINGS = {
  ...
  C: [
    { team: '阿根廷', code: 'ARG', p: 3, w: 3, d: 0, l: 0, gf: 8, ga: 1, pts: 9, qualified: true },
    { team: '奥地利', code: 'AUT', p: 3, w: 1, d: 1, l: 1, gf: 6, ga: 6, pts: 4, qualified: true },
    { team: '阿尔及利亚', code: 'ALG', p: 3, w: 1, d: 1, l: 1, gf: 6, ga: 7, pts: 4, qualified: true },
    { team: '约旦', code: 'JOR', p: 3, w: 0, d: 0, l: 3, gf: 3, ga: 9, pts: 0 },
  ],
  D: [
...
const STADIUMS = [
  { country: '阿根廷路线 · USA', stadium: 'Hard Rock Stadium', city: '迈阿密', capacity: '65,000', note: '梅西在北美俱乐部故事的城市，也是阿根廷内容最容易形成情绪连接的一站。', img: VISUALS.hardRock, license: '公开图片 / 本地兜底' },
  { country: '阿根廷路线 · USA', stadium: 'MetLife Stadium', city: '纽约/新泽西', capacity: '82,500', note: '2026 决赛场地。阿根廷卫冕叙事最终会自然指向这座球场。', img: VISUALS.metlife, license: '公开图片 / 本地兜底' },
  { country: '阿根廷路线 · MEX', stadium: 'Estadio Azteca', city: '墨西哥城', capacity: '87,000', note: '1986 阿根廷传奇记忆与 2026 美加墨世界杯在这里产生历史回声。', img: VISUALS.azteca, license: '公开图片 / 本地兜底' },
  { country: '阿根廷路线 · USA', stadium: 'SoFi Stadium', city: '洛杉矶', capacity: '70,000', note: '西海岸巨型球场，适合承载阿根廷强强对话和全球转播叙事。', img: VISUALS.sofi, license: '公开图片 / 本地兜底' },
  { country: '阿根廷路线 · USA', stadium: 'AT&T Stadium', city: '达拉斯', capacity: '80,000', note: '大容量与穹顶环境让达拉斯成为淘汰赛阶段的潜在高压节点。', img: VISUALS.att, license: '公开图片 / 本地兜底' },
  { country: '阿根廷路线 · CAN', stadium: 'BMO Field', city: '多伦多', capacity: '45,000', note: '如果阿根廷进入加拿大赛区，温差、旅行和场地节奏会成为备战变量。', img: VISUALS.bmo, license: '公开图片 / 本地兜底' },
  { country: '阿根廷路线 · CAN', stadium: 'BC Place', city: '温哥华', capacity: '54,500', note: '加拿大西海岸与封闭穹顶环境，适合展示阿根廷跨时区作战路线。', img: VISUALS.bcPlace, license: '公开图片 / 本地兜底' },
  { country: '阿根廷路线 · MEX', stadium: 'Estadio BBVA', city: '蒙特雷', capacity: '53,500', note: '墨西哥北部现代球场，能补足阿根廷在美加墨三国路线中的南线视觉。', img: VISUALS.bbva, license: '公开图片 / 本地兜底' },
];
...
function updateFixtureSource() {
  ...
  if (!sourceEl) return;
  const source = fixtureState?.source || '阿根廷动态示意';
...
function updateFocusMatch() {
  ...
  const argentinaFixtures = allFixtures.filter((f) => f.home === 'ARG' || f.away === 'ARG');
  const focusFixtures = argentinaFixtures.length ? argentinaFixtures : allFixtures;
  const now = Date.now();
  const live = focusFixtures.find((f) => f.kind === 'live');
  const upcoming = focusFixtures.find((f) => {
    if (f.kind === 'done') return false;
    const t = Date.parse(f.date || '');
    return Number.isFinite(t) && t >= now - 2 * 60 * 60 * 1000;
  });
  const recentDone = focusFixtures
    .filter((f) => f.kind === 'done')
    .sort((a, b) => Date.parse(b.date || '') - Date.parse(a.date || ''))[0];
  const next = live || upcoming || recentDone || focusFixtures[0];
  if (!next) return;
...
const KNOCKOUT_BRACKET = [
  {
    ...
    note: '阿根廷路线推演',
    matches: [
      { stage: '32 强', home: 'ARG', homeName: '阿根廷', away: 'CPV', awayName: '佛得角', city: '迈阿密', time: '7/3 18:00 ET', status: '已确认', hot: true },
      { stage: '32 强', home: 'MEX', homeName: '墨西哥', away: 'ECU', awayName: '厄瓜多尔', city: '墨西哥城', time: '赛程同步', status: '潜在对手' },
      { stage: '32 强', home: 'NED', homeName: '荷兰', away: 'KOR', awayName: '韩国', city: '北美赛区', time: '赛程同步', status: '潜在对手' },
      { stage: '32 强', home: 'POR', homeName: '葡萄牙', away: 'CRO', awayName: '克罗地亚', city: '北美赛区', time: '赛程同步', status: '潜在对手' },
      { stage: '32 强', home: 'BRA', homeName: '巴西', away: 'JPN', awayName: '日本', city: '北美赛区', time: '赛程同步', status: '潜在对手', hot: true },
      { stage: '32 强', home: 'FRA', homeName: '法国', away: 'MAR', awayName: '摩洛哥', city: '北美赛区', time: '赛程同步', status: '潜在对手', hot: true },
      { stage: '32 强', home: 'ESP', homeName: '西班牙', away: 'GER', awayName: '德国', city: '北美赛区', time: '赛程同步', status: '潜在对手' },
      { stage: '32 强', home: 'ENG', homeName: '英格兰', away: 'USA', awayName: '美国', city: '北美赛区', time: '赛程同步', status: '潜在对手' },
      { stage: '32 强', home: 'URU', homeName: '乌拉圭', away: 'COL', awayName: '哥伦比亚', city: '北美赛区', time: '赛程同步', status: '南美线' },
      { stage: '32 强', home: 'ITA', homeName: '意大利', away: 'NGA', awayName: '尼日利亚', city: '北美赛区', time: '赛程同步', status: '潜在对手' },
      { stage: '32 强', home: 'CAN', homeName: '加拿大', away: 'SEN', awayName: '塞内加尔', city: '加拿大赛区', time: '赛程同步', status: '东道主线' },
      { stage: '32 强', home: 'BEL', homeName: '比利时', away: 'SUI', awayName: '瑞士', city: '北美赛区', time: '赛程同步', status: '潜在对手' },
      { stage: '32 强', home: 'ALG', homeName: '阿尔及利亚', away: 'AUT', awayName: '奥地利', city: '北美赛区', time: '赛程同步', status: '阿根廷小组线' },
      { stage: '32 强', home: 'JOR', homeName: '约旦', away: 'AUS', awayName: '澳大利亚', city: '北美赛区', time: '赛程同步', status: '阿根廷小组线' },
      { stage: '32 强', home: 'CHI', homeName: '智利', away: 'PAR', awayName: '巴拉圭', city: '北美赛区', time: '赛程同步', status: '南美线' },
      { stage: '32 强', home: 'GHA', homeName: '加纳', away: 'JPN', awayName: '日本', city: '北美赛区', time: '赛程同步', status: '潜在对手' },
    ],
  },
  {
    round: '16 强',
    note: '潜在强强对话',
    matches: [
      { stage: '16 强', home: 'ARG', homeName: '阿根廷', away: 'NED', awayName: '荷兰 / 待定', city: '待同步', time: '自动同步', status: '推演', hot: true },
      { stage: '16 强', home: 'BRA', homeName: '巴西', away: 'POR', awayName: '葡萄牙', city: '待同步', time: '自动同步', status: '潜在对手' },
      { stage: '16 强', home: 'FRA', homeName: '法国', away: 'ESP', awayName: '西班牙', city: '待同步', time: '自动同步', status: '潜在对手' },
      { stage: '16 强', home: 'ENG', homeName: '英格兰', away: 'URU', awayName: '乌拉圭', city: '待同步', time: '自动同步', status: '潜在对手' },
    ],
  },
  {
    round: '1/4 决赛',
    note: '冠军窗口',
    matches: [
      { stage: '1/4 决赛', home: 'ARG', homeName: '阿根廷', away: 'POR', awayName: '葡萄牙 / 待定', city: '待同步', time: '自动同步', status: '推演', hot: true },
      { stage: '1/4 决赛', home: 'BRA', homeName: '巴西', away: 'FRA', awayName: '法国', city: '待同步', time: '自动同步', status: '另一半区' },
    ],
  },
  {
    round: '半决赛',
    note: '南美高压线',
    matches: [
      { stage: '半决赛', home: 'ARG', homeName: '阿根廷', away: 'BRA', awayName: '巴西 / 待定', city: '待同步', time: '自动同步', status: '推演', hot: true },
      { stage: '半决赛', home: 'FRA', homeName: '法国', away: 'ENG', awayName: '英格兰', city: '待同步', time: '自动同步', status: '另一半区' },
    ],
  },
  {
    round: '决赛',
    note: '7.19',
    matches: [
      { stage: '决赛', home: 'ARG', homeName: '阿根廷', away: 'FRA', awayName: '法国 / 待定', city: '纽约/新泽西', time: '7/19', status: '卫冕目标', hot: true },
    ],
  },
];
...
function updateScorerSource() {
  ...
  const source = /espn/i.test(String(scorerState?.source || '')) ? 'ESPN 2026 射手榜筛选' : (scorerState?.source || '阿根廷射手数据');
  const stamp = scorerState?.last_sync_at ? timeAgo(scorerState.last_sync_at) : '同步中';
  const count = scorerState?.argentina_items || 0;
  const total = scorerState?.fetched_items || 0;
  const degraded = scorerState?.degraded ? ' · 降级/待更新' : '';
  const text = `${source} · ${stamp} · 阿根廷 ${count} 人 / 全榜 ${total} 人${degraded} · 每 30 分钟自动刷新`;
  if (sourceEl) sourceEl.textContent = text;
  if (playerEl) playerEl.textContent = text;
}

function isArgentinaPlayer(item) {
  const text = `${item?.country || ''} ${item?.teamName || ''} ${item?.team || ''}`.toLowerCase();
  return codeFromCountry(item?.country) === 'ARG' || text.includes('argentina') || text.includes('阿根廷');
}
...
function applyScorers(list, state) {
  ...
  const argentinaOnly = normalized.filter(isArgentinaPlayer);
  scorerState = {
    ...scorerState,
    argentina_items: argentinaOnly.length,
    fetched_items: normalized.length || scorerState.fetched_items || 0,
  };
  renderScorers(argentinaOnly);
  renderPlayers((argentinaOnly.length ? argentinaOnly : FOCUS_PLAYERS).slice(0, 8));
  updateScorerSource();
}
...
function renderScorers(list) {
  ...
  const notice = watchMode
    ? `<p class="empty compact">2026 正赛阿根廷射手数据尚未同步到页面，以下先展示阿根廷关注名单；正式数据出现后会自动替换。</p>`
    : '';
...
const SAMPLE_NEWS = [
  {
    source: '中文新闻示例', sample: true, title: '阿根廷卫冕路线观察：梅西、斯卡洛尼和中前场选择成为核心线索',
    summary: '页面会优先追踪阿根廷队相关新闻。真实新闻源接入后，这里会自动替换为公开订阅源中的最新阿根廷内容。',
    image: VISUALS.trophy,
  },
  { source: '中文新闻示例', sample: true, title: '锋线选择：劳塔罗与阿尔瓦雷斯如何配合梅西，是阿根廷进攻观察重点', image: VISUALS.hardRock },
  { source: '中文新闻示例', sample: true, title: '中场结构：麦卡利斯特、恩佐、德保罗决定蓝白军团的推进和防守强度', image: VISUALS.soccerField },
  { source: '中文新闻示例', sample: true, title: '门线叙事：大马丁仍是阿根廷淘汰赛心理优势的关键符号', image: VISUALS.playerWalkout },
  { source: '中文新闻示例', sample: true, title: '北美路线：迈阿密、纽约/新泽西、墨西哥城构成阿根廷页面的视觉主轴', image: VISUALS.metlife },
  { source: '中文新闻示例', sample: true, title: '潜在对手雷达：法国、巴西、荷兰、葡萄牙仍是阿根廷卫冕路上的高压节点', image: VISUALS.northAmerica },
];
...
const NEWS_KEYWORD_TITLES = [
  { re: /argentina|albiceleste|scaloni|messi/i, title: '阿根廷队动态更新：梅西、斯卡洛尼和卫冕路线成为焦点' },
  { re: /lautaro|alvarez|julian|forward/i, title: '阿根廷锋线观察：劳塔罗与阿尔瓦雷斯的选择进入聚光灯' },
  { re: /enzo|mac allister|de paul|midfield/i, title: '阿根廷中场观察：控制、覆盖和推进决定比赛节奏' },
  { re: /emiliano|martinez|dibu|penalt/i, title: '阿根廷门线叙事升温，大马丁仍是淘汰赛关键人物' },
...
function synthesizeChineseSummary(item) {
  ...
  if (/argentina|albiceleste|scaloni|messi/i.test(title)) return '这条来自海外公开订阅源，页面已转为中文导览；重点关注阿根廷队阵容、状态和卫冕路线。';
  if (/penalt|shoot/i.test(title)) return '这条来自海外公开订阅源，页面已转为中文导览；重点关注淘汰赛心理压力和点球情境。';
  ...
  return item?.sample ? summary : '来自公开订阅源的阿根廷队世界杯新闻，页面以中文摘要展示，点击可阅读原文。';
}
...
const assistantMessages = [
  {
    role: 'assistant',
    text: '可以问阿根廷队 2026 世界杯赛程、阵容、梅西、斯卡洛尼、中文新闻和潜在对手，也可以要求我修改这个页面。提交后会启动后台 Session 处理。',
  },
];
...
function cleanTaggedAnswer(text) {
  ...
  const m = s.match(/<further-answering>([\s\S]*?)<\/further-answering>/i)
    ...
    || s.match(/<argentina-answer>([\s\S]*?)<\/argentina-answer>/i)
...
function renderAssistantChat() {
  ...
  wrap.innerHTML = assistantMessages.map((msg) => {
    ...
    return `<article class="assistant-msg ${escapeHtml(msg.role)}${statusClass}">
      <div class="assistant-msg-role">${msg.role === 'user' ? '你' : '阿根廷AI'}</div>
...
function updateAssistantSource(state) {
  ...
  if (!state || !state.session_id) {
    el.textContent = '阿根廷问AI 尚未启动';
    return;
  }
  const stamp = state.updated_at ? timeAgo(state.updated_at) : '最近';
  el.innerHTML = `阿根廷问AI Session：<a href="${escapeHtml(state.session_url || '#')}" target="_blank" rel="noopener noreferrer">${escapeHtml(state.session_id)}</a> · ${escapeHtml(stamp)}更新`;
}
    ...
    try {
      ...
      if (attempt < 45) {
        targetMessage.text = attempt < 2
          ? '阿根廷问AI 已启动，正在读取阿根廷队数据和插件代码。'
          : '阿根廷问AI 正在工作中，完成后会把简短结论同步到这里；也可以点开 Session 看完整进度。';
        targetMessage.status = 'pending';
        renderAssistantChat();
        pollAgentAnswer(targetMessage, attempt + 1);
        return;
      }
      targetMessage.text = '阿根廷问AI 仍在后台处理。可以打开 Session 查看完整进度，稍后也可以继续在这里追问。';
      targetMessage.status = 'pending';
      renderAssistantChat();
    } catch (e) {
      if (attempt < 8) {
...
function bindAssistantForm() {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const pending = { role: 'assistant', text: '已收到，正在投递给阿根廷问AI。后台启动后这里会显示 Session 入口，并持续同步生成状态。', status: 'pending' };
    assistantMessages.push(pending);
    renderAssistantChat();
    try {
      const r = await askWorldCupAgent(text, !!newSession?.checked);
      if (!r || !r.ok) throw new Error(r?.error || '阿根廷问AI 启动失败');
      pending.text = r.reused_session
        ? '已发送到既有阿根廷问AI Session。正在等待简短回答同步到这里。'
        : '已新建阿根廷问AI Session 并开始处理。正在等待简短回答同步到这里。';
      ...
      pending.text = `阿根廷问AI 启动失败：${e.message || e}`;
    }
    renderAssistantChat();
  });
}
...
async function loadFromBackend() {
  try {
    ...
    console.warn('[argentina-world-cup] loadFromBackend failed (likely 401 / network), keeping static demo content:', e.message || e);
    applySyncState(null);
